...
4. 未対応サイトでは「このページからToolを探す」が表示されます。クリックした現在タブだけを読み、ローカルで判定し、GitHubで実在を確かめてから通常の導入画面へ進みます。候補が複数あるページでは一覧から1件を選び、選んだ1件だけを確認します。TypeSafeへ送るのはローカルで決められなかった場合だけで、そのときは**設定**で自分のJev APIキーを登録して**AIによる検出（Beta）**を有効にする必要があります（キーを保存するまでトグルは押せず、**削除**でいつでも消せます）。送るのは候補リンク・導入コマンド断片・限定したページ文脈だけです。
...
- Jevへの通信は、利用者が未対応ページを自分で解析したときと、自動検知を許可したサイトのページのときだけで、どちらもローカル判定で決められなかった場合に限ります。HTML全文、フォーム入力値、Cookie、閲覧履歴は送りません。
