...
- 未対応サイトでは利用者が明示的に操作した場合に限り、Jev API を使った候補選択を行う（**Beta**。既定は無効で、撤去できる形で持つ）。ページ全文は送らず、ローカル抽出した GitHub リンク、導入コマンド断片、見出し等だけを送る。AI 判定だけでは導入せず、既存の GitHub 実在確認・展開確認を必ず通す。
...
- 未対応サイトの読み取りは `<all_urls>` ではなく `activeTab + scripting` を使い、利用者のクリックで現在タブにだけ一時許可する。Jev API key は `chrome.storage.local` を trusted extension contexts に制限して保存する。
